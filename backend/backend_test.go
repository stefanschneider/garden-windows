...
var _ = Describe("backend", func() {
	...
	Describe("BulkMetrics", func() {
		BeforeEach(func() {
			server.AppendHandlers(
				ghttp.CombineHandlers(
					...
					ghttp.VerifyJSONRepresenting([]string{"handle1", "handle2"}),
					ghttp.RespondWith(200, `{
						"handle1": { "Metrics": { "MemoryStat": { "TotalBytesUsed": 1234 }}},
						"handle2": { "Metrics": { "MemoryStat": { "TotalBytesUsed": 5678 }}}
					}`),
				),
			)
		})
		...
		It("returns the containers metrics", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(metrics["handle1"].Metrics.MemoryStat.TotalBytesUsed).Should(Equal(uint64(1234)))
			Expect(metrics["handle1"].Err).ShouldNot(HaveOccurred())

			Expect(metrics["handle2"].Metrics.MemoryStat.TotalBytesUsed).Should(Equal(uint64(5678)))
